...
boolean IsThereWall(Simulator x, int NCol, int NRow) {
    ...
    return !(Absis(Location(x)) >= 0 && Absis(Location(x)) <= NCol)
           && (Ordinat(Location(x)) >= 0 && Ordinat(Location(x)) <= NRow);
}

boolean IsEmptySpace(Matrix map, Point P) {
    // return (getSymbol(map, P) != 'T' && getSymbol(map, P) != 'B' && getSymbol(map, P) != 'F' && getSymbol(map, P) !=
    // 'C' && getSymbol(map, P) != 'M' && getSymbol(map, P) != 'X');
...
void SimulatorMove(Simulator *x, Point P, Matrix *map, float deltaX, float deltaY) {
    ...
    if (!IsThereWall(*x, ROW_EFF(*map), COL_EFF(*map))
        && IsEmptySpace(*map, newLocation)) {  // 10 is map size; GANTI 10 DENGAN MAP SIZE!
       ...
       //  getSymbol(*map, newLocation)); if(IsEmptySpace(*map, newLocation) && IsThereWall(*x, 10, 10))
...
boolean IsBuySpace(Matrix map, Point P) {
    ...
    // TulisPOINT(Bawah);
    // printf("\n");
    return (getSymbol(map, Kanan) == 'T' || getSymbol(map, Kiri) == 'T' || getSymbol(map, Bawah) == 'T'
            || getSymbol(map, Atas) == 'T');
};
...
boolean IsMixSpace(Matrix map, Point P) {
    ...
    return (getSymbol(map, Kanan) == 'M' || getSymbol(map, Kiri) == 'M' || getSymbol(map, Bawah) == 'M'
            || getSymbol(map, Atas) == 'M');
};
...
boolean IsBoilSpace(Matrix map, Point P) {
    ...
    return (getSymbol(map, Kanan) == 'B' || getSymbol(map, Kiri) == 'B' || getSymbol(map, Bawah) == 'B'
            || getSymbol(map, Atas) == 'B');
};
...
boolean IsFrySpace(Matrix map, Point P) {
    ...
    return (getSymbol(map, Kanan) == 'F' || getSymbol(map, Kiri) == 'F' || getSymbol(map, Bawah) == 'F'
            || getSymbol(map, Atas) == 'F');
};
...
boolean IsChopSpace(Matrix map, Point P) {
    ...
    return (getSymbol(map, Kanan) == 'C' || getSymbol(map, Kiri) == 'C' || getSymbol(map, Bawah) == 'C'
            || getSymbol(map, Atas) == 'C');
};
...
boolean IsFridgeSpace(Matrix map, Point P) {
    ...
    return (getSymbol(map, Kanan) == 'R' || getSymbol(map, Kiri) == 'R' || getSymbol(map, Bawah) == 'R'
            || getSymbol(map, Atas) == 'R');
}
