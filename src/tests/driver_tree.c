...
int main(){ 
    ...
    for(i = 0; i<teschilcount; i++){
        ...
        CreateFood(&ayam[i], uniqname, dmytm, acloc, dmytm);
    }

    Food gigachadFood; CreateFood(&gigachadFood, StringFrom("parent ayam"), dmytm, acloc, dmytm);
    Node basse = *newNode(&gigachadFood); // lets use this
    Tree trie; CreateTree(&trie, &basse);

    
    for(i = 0; i<teschilcount; i++){
        childee[i] = *newNode(&ayam[i]);
    }
